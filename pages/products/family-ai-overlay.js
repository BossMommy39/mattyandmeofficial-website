...
const CHALLENGES = [
  ...
  { icon: "🏠", label: "Morning/evening routines" },
  { icon: "🍽️", label: "Food battles" }
];
...
const BOUNDARIES = [
  "Compare siblings to each other", "Suggest punishments or consequences",
  "Recommend activities without parent approval", "Give medical or safety advice",
  "Push religious/political views"
];
...
export default function ProductPage() {
  ...
  const addMember = useCallback(() => {
    if (!currentMember.name || !currentMember.age) return;
    update("members", [...family.members, currentMember]);
    setCurrentMember({ name: "", age: "", role: "Kid", personality: [], superpower: "", comm: "", challenge: "" });
    setAdding(false);
  }, [currentMember, family.members, update]);

  const removeMember = useCallback((index) => {
    update("members", family.members.filter((_, i) => i !== index));
  ...
  const buildScript = useCallback(() => {
    ...
    script += "\nFAMILY DYNAMICS:\n";
    if (family.conflict) script += `- Conflict resolution: ${family.conflict}\n`;
    if (family.sibling && family.members.length > 1) script += `- Sibling dynamics: ${family.sibling}\n`;
    if (family.stress) script += `- Crisis response: ${family.stress}\n`;

    script += `\nCURRENT FOCUS: Help with ${family.challenges.join(", ")}\n`;
    script += `\nFAMILY VALUES: ${family.values.join(", ")}\n`;
    
    script += "\nNEVER DO:\n";
    family.boundaries.forEach((b) => script += `- ${b}\n`);
    if (family.customBoundary) script += `- ${family.customBoundary}\n`;
          ...
          <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="bg-[#f5f1eb] rounded-lg p-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {VIBES.map(vibe => (
                          <button 
                            key={vibe.label} 
                            type="button"
                            onClick={() => update("vibe", vibe.label)}
                    ...
                    <div className="bg-blue-50 border-2 border-[#214179] rounded-lg p-6 mt-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        >
                          ...
                          {COMMS.map(style => <option key={style} value={style}>{style}</option>)}
                        </select>
                      </div>
                      ...
                      <input 
                        ...
                        className="w-full p-2 border rounded focus:border-[#214179] focus:outline-none"
                                   />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select 
                        value={family.techComfort} 
                        onChange={e => update("techComfort", e.target.value)}
                        className="w-full p-2 border rounded focus:border-[#214179] focus:outline-none"
                      >
                        <option value="">How comfortable are you?</option>
                        <option>Very comfortable - just give me the script</option>
                        <option>Somewhat comfortable - basic instructions needed</option>
                        <option>Need hand-holding - detailed setup guide please</option>
                        <option>Technophobic - I'll need video tutorials</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <select 
                        value={family.chatgpt} 
                        onChange={e => update("chatgpt", e.target.value)}
                        className="w-full p-2 border rounded focus:border-[#214179] focus:outline-none"
                      >
                        <option value="">Current ChatGPT usage?</option>
                        <option>I already use ChatGPT regularly</option>
                        <option>I have an account but rarely use it</option>
                        <option>I'll need to create an account</option>
                        <option>What's ChatGPT?</option>
                      </select>
                    </div>
                  </div>
                </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-gray-50 rounded-lg p-6 sticky top-6">
                ...
                {/* Preview of personalized response */}
              ...
              <div className="p-8">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="border-2 border-[#214179] rounded-lg p-6 relative">
                    ...
                    {/* PayPal Button for $20 */}
                    <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
                      <input type="hidden" name="cmd" value="_s-xclick" />
                      <input type="hidden" name="hosted_button_id" value="YOUR_20_BUTTON_ID" />
                      ...
                      <button 
                        type="submit"
                        className="w-full bg-[#214179] text-white py-3 px-6 rounded-lg font-semibold hover:bg-[#826753] transition-colors"
                      >
                        Get Started - $20
                      </button>
                    </form>
                  ...
                  <div className="border-2 border-[#826753] rounded-lg p-6 relative">
                    ...
                    {/* PayPal Button for $67 */}
                    <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
                      <input type="hidden" name="cmd" value="_s-xclick" />
                      <input type="hidden" name="hosted_button_id" value="YOUR_67_BUTTON_ID" />
                      <input type="hidden" name="custom" value={JSON.stringify({ 
                        tier: 'premium', 
                        familyData: family,
                        script: buildScript()
                      ...
                      <button 
                        type="submit"
                        className="w-full bg-[#826753] text-white py-3 px-6 rounded-lg font-semibold hover:bg-[#214179] transition-colors"
                      >
          ...
          <div className="max-w-4xl mx-auto px-6 text-center">
            ...
            <p className="text-sm text-gray-500">
              Zero engagement theater. Real results. Field-tested technology, not startup promises.
...
} rounded focus:border-[#214179] focus:outline-none mb-4"
                      />
                ...
                <div className="mb-8">
                  <h3 className="text-xl font-semibold text-[#214179] mb-4 border-b-2 border-[#826753] pb-2">
                    Family Dynamics <span className="text-gray-500 text-base font-normal">(optional)</span>
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                        ...
                        <option>Family meetings - everyone talks it out</option>
                        <option>Parents listen to input, then decide</option>
                        <option>Case-by-case depending on situation</option>
                        <option>Parents handle it, kids follow decision</option>
                      </select>
                    </div>
                    ...
                    {family.members.length > 1 && (
                      <div>
                        ...
                        <select 
                          ...
                          onChange={e => update("sibling", e.target.value)}
                          className="w-full p-2 border rounded focus:border-[#214179] focus:outline-none"
                        >
                          <option value="">Select relationship...</option>
                          <option>Best friends who occasionally fight</option>
                          <option>Typical rivalry but have each other's backs</option>
                          <option>Constant competition - everything's a contest</option>
                          <option>Very different personalities, separate worlds</option>
                ...
                {/* Section 5: Boundaries & Tech */}
                <div className="mb-8">
                  <h3 className="text-xl font-semibold text-[#214179] mb-4 border-b-2 border-[#826753] pb-2">
                    AI Boundaries & Tech Setup
                  </h3>
                  
                  <div className="mb-4">
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      What should AI never do or suggest?
                    </label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
                      {BOUNDARIES.map(boundary => (
                        <label key={boundary} className="flex items-center gap-2 text-sm">
                          <input 
                            type="checkbox"
                            checked={family.boundaries.includes(boundary)}
                            onChange={e => update("boundaries",
                              e.target.checked
                                ? [...family.boundaries, boundary]
                                : family.boundaries.filter(b => b !== boundary)
                            )}
                    ...
                    <input 
                      type="text"
                      value={family.customBoundary} 
                      onChange={e => update("customBoundary", e.target.value)}
                      placeholder="Other boundaries (custom)"
